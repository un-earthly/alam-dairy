...
export default function FarmResults({
  initialProducts,
  initialHasMore,
  basePath,
}: {
  ...
  async function loadMore() {
    ...
    try {
      ...
      const res = await fetch(`/api/farm/products?${params.toString()}`)
    ...
    return (
      <div className="relative mt-12 flex flex-col items-center justify-center gap-4 overflow-hidden rounded-2xl py-20 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {locale === 'bn' ? 'অন্য ফিল্টার চেষ্টা করুন' : 'Try adjusting your filters'}
        </p>
      </div>
    )
  }
